...
function Home() {
  ...
  const { data, isLoading, isSuccess, isError, refetch } = useGetCustomersQuery(
    setTokenInJson,
    {
      skip: !setTokenInJson, // skip query if no token
    }
  );
  ...
  const handleDelete = async (id, imageCode) => {
      ...
      if (data?.error?.message === "Failed to fetch Data") {
        ...
        content = data?.success?.data?.map((customer, index) => (
          <tr key={index}>
            ...
            {userAuth[0]?.role === "admin" || userAuth[0]?.role === "editor" ? (
              <td>
                <div role="group" className="btn-group btn-group-sm commonBtn">
                  ...
                  {userAuth[0]?.role === "admin" && (
                    <button
                      type="button"
                      onClick={() =>
                        handleDelete(customer._id, customer.imageCode)
                      }
