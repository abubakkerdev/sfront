...
function UserEdit() {
  ...
  const { data, isLoading, isSuccess, isError, refetch } = useGetCustomerQuery(
    ...
    {
      skip: !setTokenInJson || !id,
    }
  );

  const { refetch: customersRefetch } = useGetCustomersQuery(setTokenInJson, {
    skip: !setTokenInJson,
  });
    ...
    if (data && !isError) {
      if (data && "success" in data) {
        ...
        setUserInput({
          ...
          imageCode: data.success.data[0].imageCode,
  ...
  const handleUpdateInformation = async (e) => {
    ...
    try {
      await updateCustomer({
        data: {
          ...userInput,
          imageObj: [imageBase64Data, imageInfo, userInput.imageCode],
        },
        token: setTokenInJson,
      }).unwrap();
    } catch (error) {
