...
import { deleteToken } from "../features/user/tokenSlice";
...
function ChangePassword() {
  ...
  const userAuthToken = useSelector((state) => state.tokenInfo.token);
  ...
  useEffect(() => {
    if (userAuth.length > 0) {
      ...
      if (token !== userAuthToken.forgotToken) {
        navigate("/login");
      }
    }
  }, [navigate, userAuth, userAuthToken]);
  ...
  useEffect(() => {
    if (data && !isError) {
      if (data && "success" in data) {
        ...
        dispatch(deleteToken({ forgotToken: "101010" }));
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      await changePassword({
        data: { ...changePasswordInfo, ...userAuthToken },
